...
// MsgReq 客户端发送聊天消息的载荷
type MsgReq struct {
	ReceiverID uint64 `json:"receiver_id"` // 接收人ID (品牌发给红人，或红人发给品牌)
...
type KafkaChatMsg struct {
	...
	SenderID   uint64 `json:"sender_id"`   // 消费端要知道是谁发的
	ReceiverID uint64 `json:"receiver_id"` // 消费端要靠它去本地连接池找接收人
	MsgType    int8   `json:"msg_type"`
	Content    string `json:"content"`
